...
- metadata_schema and biological_context tell you what covariates actually exist in adata.obs. Treat any low-cardinality column whose name or values resemble batch, donor, sample, replicate, or platform identifiers as a candidate batch key. Treat biological label columns such as final_annotation or cell_type as existing labels that may be reused for comparison or validation. They are not evidence that computational clustering, neighborhood graphs, or differential expression have been completed.
...
EXPLICIT ANNOTATION PREREQUISITE RULE

When the user explicitly requests cell-type annotation, comparison, or consensus, do not treat final_annotation, cell_type, or any other biological label column as a substitute for computational clusters or marker genes. If metadata_schema.obs_columns does not contain a real cluster/group column such as leiden, louvain, cluster, or clusters, schedule PCA when needed, neighborhood-graph construction, clustering, and cluster-level differential expression before the annotation tools. Preserve existing biological labels for comparison, but do not use them as the cluster_key for ACT or SCSA and do not claim that their prerequisites are satisfied.
